...
const RECOMMENDED_STYLES = [
  {
    id: 101,
    name: 'Ivory Floral Organza Saree',
    price: 11500,
  ...
  {
    id: 102,
    name: 'Crimson Velvet Dupatta',
    price: 4800,
    image: '/images/bridal_dupatta.jpg',
    category: 'Dupattas',
    subtitle: 'Heritage gold zardozi border',
    badge: 'Bridal',
  },
  {
    id: 103,
    name: 'Pearl White Zardosi Kurta',
    price: 9200,
    image: '/images/everyday_kurta_set.jpg',
    category: 'Kurtas',
    subtitle: 'Artisanal gota patti craft',
    badge: 'Trending',
  },
  {
    id: 104,
    name: 'Rose Pink Chanderi Suit',
    price: 8600,
    image: '/images/floral_organza_saree.jpg',
    category: 'Suits',
    subtitle: 'Pure handloom cotton-silk',
    badge: 'New Arrival',
  },
  {
    id: 105,
    name: 'Royal Sapphire Evening Gown',
    price: 14500,
...
export default function Cart() {
  ...
  const handleApplyCoupon = async (e) => {
    ...
    try {
      ...
      // Fallback demo coupon support
      if (code === 'AGVIA10' || code === 'WELCOME10') {
        const disc = Math.round(selectedSubtotal * 0.1)
        setDiscountAmount(disc)
        setAppliedCoupon(`${code} (-₹${disc.toLocaleString('en-IN')})`)
        setAppliedCouponCode(code)
        toast.success(`Coupon ${code} applied successfully! (10% OFF)`, {
          style: { background: '#8B0000', color: '#FFFDF8', borderRadius: '12px' },
        })
        setCouponCode('')
      } else {
        toast.error('Invalid or expired coupon code. Try AGVIA10')
      }
    } finally {
      setValidating(false)
    }
    ...
    <div className="min-h-screen bg-[#FAF7F2] text-[#211D1E] font-sans antialiased selection:bg-[#5A1020] selection:text-white">
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        <div className="mb-8 md:mb-12">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-[#E6C687]/30">
            {/* Left: Your Cart Title */}
            <div>
              <h1
                style={{ fontFamily: "'Cormorant Garamond', 'Playfair Display', Georgia, serif" }}
                className="text-4xl md:text-5xl font-bold text-[#211D1E] tracking-tight leading-tight"
              >
                Your{' '}
                <span
              ...
              <p
                ...
                className="text-xs sm:text-sm text-[#211D1E]/65 mt-1.5"
              >
                Handpicked pieces, ready to be yours.
              </p>
              {/* Gold underline motif */}
              <div className="flex items-center gap-2 mt-2">
                <span className="w-10 h-[1.5px] bg-[#C9A45C]" />
                <span className="w-1.5 h-1.5 rotate-45 bg-[#C9A45C]" />
                <span className="w-4 h-[1.5px] bg-[#C9A45C]/50" />
              </div>
            </div>

            {/* Center: 3-Step Luxury Stepper */}
            <div className="flex items-center justify-center gap-3 sm:gap-6 self-center my-2 lg:my-0">
              {/* Step 1: Cart (Active) */}
              <div className="flex flex-col items-center gap-1.5">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-[#5A1020] text-white flex items-center justify-center shadow-[0_4px_12px_rgba(90,16,32,0.25)] ring-2 ring-[#C9A45C]/40">
                  <ShoppingBag size={16} />
                </div>
                <span
                  style={{ fontFamily: "'Lato', sans-serif" }}
                  className="text-[11px] sm:text-xs font-bold text-[#5A1020] tracking-wide"
                >
                  1. Cart
                </span>
              </div>
              ...
              {/* Connector */}
              <div className="w-10 sm:w-16 h-[1.5px] bg-[#C9A45C]/40 mb-4" />

              {/* Step 2: Details */}
              <div className="flex flex-col items-center gap-1.5 opacity-60">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white border border-[#C9A45C]/40 text-[#211D1E]/60 flex items-center justify-center shadow-xs">
                  <Truck size={16} />
                </div>
                <span
                  style={{ fontFamily: "'Lato', sans-serif" }}
                  className="text-[11px] sm:text-xs font-medium text-[#211D1E]/70"
                >
                  2. Details
                </span>
              </div>
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 space-y-4">
              {/* Selection Bar */}
              <div className="bg-white rounded-2xl p-4 border border-[#E6C687]/30 shadow-xs flex items-center justify-between">
                <label className="flex items-center gap-3 cursor-pointer select-none">
                  <input
                    type="checkbox"
                    checked={allSelected}
                    onChange={handleToggleSelectAll}
                    className="w-4 h-4 rounded text-[#5A1020] accent-[#5A1020] focus:ring-0 cursor-pointer"
                  />
                  <span className="text-xs sm:text-sm font-bold text-[#211D1E]">
                    Select All{' '}
                    <span className="font-normal text-[#211D1E]/60">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs text-[#211D1E]/60 hover:text-red-700 transition-colors cursor-pointer"
                >
                  <Trash2 size={14} />
                  ...
                  {items.map((item, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        key={item.id}
                        className={`bg-white rounded-2xl p-4 sm:p-5 border transition-all duration-300 relative shadow-xs hover:shadow-md ${
                      ...
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-5">
                          ...
                          {/* Thumbnail Image */}
                          <div className="relative w-24 h-28 sm:w-28 sm:h-32 rounded-xl overflow-hidden bg-[#FAF7F2] border border-[#E6C687]/30 shrink-0 shadow-xs">
                            <ReliableImage
                              src={item.image}
                              alt={item.name}
                          ...
                          <div className="flex-1 min-w-0">
                            <h3
                              style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                              className="text-base sm:text-lg font-bold text-[#211D1E] leading-snug line-clamp-1"
                            >
                              {item.name}
                            </h3>
                            <p className="text-xs text-[#211D1E]/60 mt-0.5 line-clamp-1">
                              {item.category || 'Luxury Silhouettes'}
                            </p>

                            {/* Color attribute & swatches */}
                            <div className="flex items-center gap-2 mt-2 text-xs text-[#211D1E]/75">
                              <span className="text-[11px] font-medium text-[#211D1E]/70">
                              ...
                              <div className="flex items-center gap-1.5 ml-1">
                                {swatches.map((color, sIdx) => (
                                  <span
                                    ...
                                    className={`w-3.5 h-3.5 rounded-full inline-block border border-white shadow-xs ${
                            ...
                            {/* Size attribute */}
                            <p className="text-[11px] text-[#211D1E]/70 mt-1">
                              Size: <span className="font-semibold text-[#211D1E]">{sizeLabel}</span>
                            </p>
                          </div>

                          {/* Right Controls: Badge, Price, Quantity, Wishlist, Trash */}
                          <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-3 sm:gap-2.5 shrink-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-[#E6C687]/20">
                            {/* Badge */}
                            {badgeText && (
                              <span
                                style={{ fontFamily: "'Lato', sans-serif" }}
            ...
            <div className="lg:col-span-4 space-y-6">
              {/* Order Summary Card */}
              <div className="bg-white rounded-2xl p-6 md:p-7 border border-[#E6C687]/35 shadow-sm space-y-5">
                <div className="flex items-center justify-between pb-3 border-b border-[#E6C687]/20">
                  <h2
                    style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                    className="text-xl font-bold text-[#211D1E]"
                  >
                    Order Summary
                  </h2>
                  <span className="text-xs text-[#211D1E]/60 font-medium">
                    {selectedItems.length} item{selectedItems.length !== 1 ? 's' : ''}
                  </span>
                </div>

                {/* Subtotals */}
                <div className="space-y-3 text-xs sm:text-sm text-[#211D1E]/75">
                  ...
                  <div className="border-t border-[#E6C687]/25 pt-3.5 flex justify-between items-baseline">
                    <span
                      style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                    ...
                    <span
                      style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                      className="text-2xl font-bold text-[#5A1020]"
                    >
                ...
                <button
                  ...
                  className={`w-full py-3.5 px-6 rounded-full font-semibold text-xs tracking-wider uppercase flex items-center justify-center gap-2 transition-all duration-300 shadow-md ${
                    selectedItems.length === 0
                      ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                      : 'bg-[#5A1020] text-white hover:bg-[#8B0000] hover:shadow-lg active:scale-[0.99]'
                  }`}
                >
                  <Lock size={13} />
                ...
                <Link
                  to="/products"
                  className="w-full py-3 px-6 rounded-full border border-[#5A1020]/25 text-[#5A1020] hover:border-[#5A1020] hover:bg-[#FAF7F2] font-semibold text-xs tracking-wider uppercase flex items-center justify-center gap-2 transition-all duration-200"
                >
                  ← Continue Shopping
                </Link>
                ...
                {/* Coupon Code Section */}
                <div className="pt-2">
                  <div className="p-3.5 rounded-xl border border-[#E6C687]/30 bg-[#FAF7F2]/60 space-y-2.5">
                    <div className="flex items-center gap-2">
                      <div className="w-5 h-5 rounded-full bg-[#C9A45C]/20 text-[#8B0000] flex items-center justify-center shrink-0">
                        <Percent size={11} />
                      </div>
                      <span className="text-xs font-bold text-[#211D1E]">Apply Coupon Code</span>
                    </div>
        ...
        <section className="mt-14 md:mt-20 pt-10 border-t border-[#E6C687]/30">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2
                style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}
                className="text-2xl sm:text-3xl font-bold text-[#5A1020]"
              >
                You May Also Like
              </h2>
          ...
          <div className="relative group/rec">
            ...
            <button
              ...
              className="absolute -left-3 sm:-left-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/95 border border-[#E6C687]/50 shadow-md flex items-center justify-center text-[#5A1020] hover:bg-[#5A1020] hover:text-white transition-all"
            ...
            <button
              onClick={() => scrollCarousel(1)}
              className="absolute -right-3 sm:-right-4 top-1/2 -translate-y-1/2 z-20 w-9 h-9 rounded-full bg-white/95 border border-[#E6C687]/50 shadow-md flex items-center justify-center text-[#5A1020] hover:bg-[#5A1020] hover:text-white transition-all"
            ...
            >
              {RECOMMENDED_STYLES.map((prod) => (
                <div
                  key={prod.id}
                  className="shrink-0 w-[220px] sm:w-[250px] bg-white rounded-2xl overflow-hidden border border-[#E6C687]/30 shadow-xs hover:shadow-md transition-all group flex flex-col justify-between"
                >
